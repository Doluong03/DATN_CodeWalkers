window.CategoryController = function ($scope, $http, $window, $timeout) {
  $scope.listCategory = [];
  $scope.pageNo = 0;
  $scope.sizePage = 5;
  ...
  $scope.addCategory = function (event) {
    if (!$scope.formCategory.name
      || !$scope.formCategory.status
    ) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        $http.post(apiAdmin + "Category/insert", JSON.stringify($scope.formCategory), headers)
  ...
  $('#categoryUpdateModal').on('hidden.bs.modal', function (e) {
    // Thực hiện hành động khi modal đóng
    $scope.formCategoryUpdate = {};
});
  ...
  $scope.UpdateCategory = function (event) {
    if (!$scope.formCategoryUpdate.name
      || !$scope.formCategoryUpdate.status
    ) {
      // Hiển thị thông báo lỗi
      $scope.checkAddress = true;
      return; // Dừng việc thực hiện lưu nếu thông tin không hợp lệ
    ...
    event.preventDefault();
    console.log("aaa"+$scope.formCategoryUpdate);
  ...
  $scope.deleteAll = function () {
    var selectedItems = $scope.listCategory.filter(function (item) {
  ...
  $timeout(function () {
    var thElements = document.querySelectorAll('#CategoryTable th:not(:last-child)'); // Loại bỏ cột "Action"
