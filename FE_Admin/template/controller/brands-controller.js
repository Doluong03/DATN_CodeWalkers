window.BrandsController = function ($scope, $http, $window, $timeout) {
  $scope.listBrands = [];
  $scope.pageNo = 0;
  $scope.sizePage = 5;
  ...
  $scope.addBrands = function (event) {
    ...
    if (!$scope.formBrands.name
      || !$scope.formBrands.description
    ) {
  ...
  $scope.UpdateBrands = function (event) {
    ...
    if (!$scope.formBrandsUpdate.name
      || !$scope.formBrandsUpdate.description
      || !$scope.formBrandsUpdate.status
    ) {
      // Hiển thị thông báo lỗi
  ...
  $scope.exportToExcel = function () {
    // Lấy bảng theo ID
    var table = document.getElementById("BrandsTable"); // Thay id table bảng của bạn vào đây
    ...
    for (var i = 0; i < table.rows.length; i++) {
      ...
      for (var j = 1; j < table.rows[i].cells.length - 1; j++) {
        ...
        if (i!==0 && j === table.rows[i].cells.length - 2) {
    ...
    // Xuất file Excel
    XLSX.writeFile(wb, "exported_data_brand.xlsx");
  };
  ...
  $scope.selectAllChanged = function () {
    ...
    angular.forEach($scope.listBrands, function (item) {
      item.isSelected = $scope.selectAllCheckbox;
    });
  };

  $scope.deleteAll = function () {
    var selectedItems = $scope.listBrands.filter(function (item) {
  ...
  $timeout(function () {
    var thElements = document.querySelectorAll('#BrandsTable th:not(:last-child)'); // Loại bỏ cột "Action"
